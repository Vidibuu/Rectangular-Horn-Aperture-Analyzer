...
def compute_pattern_1d(plane: str, lam: float, a1_eff: float, b1_eff: float, lE: float, lH: float, N: int):
    """
    Cortes principales EXACTOS (Balanis, 3rd ed.):
...
# === Added: helper to reuse EXACT 1D method for HPBW in polar views ===
...
def compute_overlay_at_fixed_phi(plane: str, lam: float, a1_eff: float, b1_eff: float, lE: float, lH: float, a_feed: float, b_feed: float, N: int):
    """Overlay curve for principal cuts using the *same physics* as 'cortes arbitrarios':
...
def compute_pattern_polar(plane: str, lam: float, a1_eff: float, b1_eff: float, lE: float, lH: float, N: int):
    """
    Versión polar de los cortes principales, idéntica a compute_pattern_1d pero barriendo 0..180°.
    """
    import numpy as _np
    from scipy.special import fresnel as _fresnel
...
def _univ_H_db(t, x_vec):
    """Universal curves for H-plane sectoral horn (Balanis 3e, eq. 13-33).
    Inputs: t = a1^2/(8 λ ρ2), x_vec = (a1/λ)*sinθ
    Output: dB, normalized to 0 dB max (excludes (1+cosθ) factor).
    """
    import numpy as _np
    from scipy.special import fresnel as _fresnel
    x_vec = _np.asarray(x_vec, dtype=float)
...
def fields_sectorial_E(theta, phi, lam, b1, rho1, a):
    """
    Far-field for sectorial E horn at arbitrary φ.
    Reduces to principal E-plane when φ=90° (sinc term → 1).
    """
    th = theta
    ph = phi
    k = 2 * np.pi / lam
...
def fields_sectorial_H(theta, phi, lam, a1, rho2, b):
    """
    Far-field for sectorial H horn at arbitrary φ.
    Reduces to principal H-plane when φ=0° (sinc term → 1).
    """
    th = theta
    ph = phi
    k = 2 * np.pi / lam
    ...
    if submitted:
        ...
        if tipo_sel == 'Piramidal':
            if modo_plot == _T(st.session_state.lang, 'mode_1d'):
                ...
                # === Added: show HPBW (same as 1D) for polar E & H ===
                try:
            ...
            if modo_plot == _T(st.session_state.lang, 'mode_1d'):
                ...
                # === Added: show HPBW (same as 1D) for polar single-plane ===
                try:
